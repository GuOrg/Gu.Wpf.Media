...
namespace Gu.Wpf.Media.Tests.CodeGen
{
    using System;
    using System.IO;
    using System.Linq;
    ...
    public class ResharperPattern
    {
        public void TestMethod()
        {
            ...
            wrapper.IsMuted = true;
        }

        [Test]
        public void DotSettingsHasCorrectPattern()
        {
            var pattern = this.MutablePropertiesRegex<MediaElementWrapper>();
        ...
        public void WriteUserDotSettings()
        {
            ...
            using (var writer = new StreamWriter(File.OpenWrite(file.FullName)))
            {
                ...
                writer.Write(this.GetResharperPattern<MediaElementWrapper>());
    ...
    where T : DependencyObject
        {
            var mutableDps = typeof(T).GetFields(BindingFlags.Static | BindingFlags.Public)
